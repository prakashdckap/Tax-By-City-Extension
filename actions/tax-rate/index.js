  ...
  try {
    ...
    if (limit !== null) {
      try {
        ...
        // Get total count
        const totalItems = await dbHelper.countTaxRates(filter, region);
        ...
        const items = await dbHelper.findTaxRates(filter, options, region);
...
async function main(params) {
  ...
  try {
    ...
    if (operation === 'DELETE') {
      ...
      const result = await dbHelper.deleteTaxRate(deleteFilter, region);
    ...
    if (operation === 'CREATE' || operation === 'POST') {
      ...
      const result = await dbHelper.insertTaxRate(taxRateData, region);
    ...
    if (operation === 'UPDATE' || operation === 'PUT') {
      ...
      const result = await dbHelper.updateTaxRate(updateFilter, update, region);
      
      if (result.success) {
        // Fetch updated document
        const updatedDoc = await dbHelper.findOneTaxRate(updateFilter, region);
